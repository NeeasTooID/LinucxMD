...
let handler = async (m, { conn, text, command }) => {
    ...
    const nazz_cmd = {
        ...
        join: () => {
            ...
            if(Object.keys(ut[m.chat].players).length >= 4) return m.reply("Player sudah melebihi limit!");
            if(ut[m.chat].status === 'PLAYING') return m.reply("Game sedang berjalan, tidak dapat bergabung");
        ...
        delete: () => {
            if(!ut[m.chat]) return m.reply("Tidak ada sesi permainan di chat ini!");
            if((ut[m.chat].host !== m.sender) && ((Date.now() - ut[m.chat].date) < 300000)) return m.reply("Anda tidak dapat menghapus sesi permainan, karena anda bukanlah host, anda dapat menghapus session setelah "+timeToFixed(300000 - (Date.now() - ut[m.chat].date)))
            if((ut[m.chat].host !== m.sender) && (ut[m.chat].status === 'PLAYING') && ((Date.now() - ut[m.chat].date) < 1000000)) return m.reply("Anda tidak dapat menghapus sesi permainan, karena anda bukanlah host dan permainan sedang berlangsung, anda dapat menghapus session setelah "+timeToFixed(1000000 - (Date.now() - ut[m.chat].date)))
            delete ut[m.chat];
...
${Object.keys(ut[m.chat].players).map(v => "- @"+v.split("@")[0]).join("\n") }`, mentions: Object.keys(ut[m.chat].players) }, { quoted: m }) 
       ...
       exit: () => {
       	if(!ut[m.chat]) return m.reply("Tidak ada sesi permainan di chat ini!");
       if(!Object.keys(ut[m.chat].players).includes(m.sender)) return m.reply("Anda tidak bergabung di pemainan!");
...
async function kocok(m, ut, conn) {
        ...
        if(players.indexOf(m.sender) !== turns) return m.reply("Bukan giliran anda!");
