...
Items: ${Object.keys(inventory.items).map(v => user[v]).reduce((a, b) => a + b, 0)} Items` : ''}${crates ? `

${crates}

Crates: ${Object.keys(inventory.crates).map(v => user[v]).reduce((a, b) => a + b, 0)} Crates` : ''}${pets || user.petFood ? `

${pets ? pets + '\n' : ''}${user.petFood ? 'Pet Food: ' + user.petFood : ''}` : ''}${cooldowns ? `
